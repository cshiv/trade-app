...
class ZerodhaUser():
    ...
    def load_user_details(self,user):
        user_details = {}
        fields = ["user","password","mfa","api_key","api_secret","access_token"]
        for item in fields:
            user_details[item]=aws_ssm.get_value("/"+"/".join([self.zerodha_config["name"],user,item]))
        return user_details
    ...
    def generate_access_token(self):
        try:
            ...
            aws_ssm.set_value("/"+"/".join([self.zerodha_config["name"],self.user,"access_token"]),self.access_token)
